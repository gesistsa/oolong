...
#' @export
summarize_oolong_gold_standard <- function(..., target_value = NULL, plot = TRUE) {
    obj_list <- list(...)
...
#' @export
print.oolong_res_gs <- function(res, plot = TRUE) {
    if (plot & !is.null(res$diag_plot)) {
        print(res$diag_plot)
    }
    .cp(!is.null(res$kripp), "Krippendorff's Alpha: ", res$kripp$value)
    .cp(!is.null(res$cor), "Correlation: ", round(res$cor$estimate, 3), " (p = ", round(res$cor$p.value,3), ")")
    .cp(!is.null(res$cor_length), "Effect of content length: ", round(res$cor_length$estimate, 3), " (p = ", round(res$cor_length$p.value,3), ")")
}
