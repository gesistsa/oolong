...
| function | purpose |
|---:|:---|
| [`wi()`](https://gesistsa.github.io/oolong/reference/create_oolong.md) | validating a topic model with [word intrusion test](#word-intrusion-test) (Chang et al., 2008) |
| [`ti()`](https://gesistsa.github.io/oolong/reference/create_oolong.md) | validating a topic model with [topic intrusion test](#topic-intrusion-test) (Chang et al., 2008; aka “T8WSI” in Ying et al. 2021) |
| [`witi()`](https://gesistsa.github.io/oolong/reference/create_oolong.md) | validating a topic model with [word intrusion test](#word-intrusion-test) and [topic intrusion test](#topic-intrusion-test) |
| [`wsi()`](https://gesistsa.github.io/oolong/reference/create_oolong.md) | validating a topic model with [word set intrusion test](#word-set-intrusion-test) (Ying et al. 2021) |
| [`gs()`](https://gesistsa.github.io/oolong/reference/create_oolong.md) | oolong test for [creating gold standard](#creating-gold-standard) (see Song et al., 2020) |
...
`abstracts_seededlda` is an example topic model trained with the data
`abstracts` using the `seededlda` package. Currently, this package
supports structural topic models / correlated topic models from `stm`,
Warp LDA models from `text2vec` , LDA/CTM models from `topicmodels`,
Biterm Topic Models from `BTM`, Keyword Assisted Topic Models from
`keyATM`, and seeded LDA models from `seededlda`. Although not strictly
a topic model, Naive Bayes models from `quanteda.textmodels` are also
supported. See the section on [Naive Bayes](#about-naive-bayes) for more
information.
...
For example, `abstracts_seededlda` was generated with the corpus
`abstracts$text`
...
#> (H0: Median TLO is not better than random guess): 0.114
```
...
- Subplot (top left): Raw correlation between human judgement and target
  value. One should want to have a good correlation between the two.
...
- Subplot (bottom right): Cook’s distance of all data point. One should
  want to have no dot (or at least very few dots) above the threshold.
  It is an indication of how the raw correlation between human judgement
  and target value can or cannot be influenced by extreme values in your
  data.
...
al. (2020) suggest Krippendorff’s Alpha \> 0.7 as an acceptable cut-off.
...
![](overview_files/figure-html/diagnosis-1.png)
...
purposes. If you still need to use
[`create_oolong()`](https://gesistsa.github.io/oolong/reference/create_oolong.md),
the most important parameters are `input_model` and `input_corpus`.
Setting each of them to `NULL` generates different tests.
